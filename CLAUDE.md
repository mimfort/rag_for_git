...
- **Исчерпанный пул — деталь внутри класса, а мёртвый эмбеддер — свой класс (PRI-274 + PRI-272).**
  ...
  `missing_database`). Эмбеддер, наоборот, получает равноправный `cause:
  ...
  тревогу на каждом втором прогоне. Иерархия `voyageai.error` плоская — `RateLimitError` не
  наследник `APIError`, поэтому исключение выражается вторым `isinstance`, а не порядком веток.
