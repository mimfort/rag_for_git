...
- **Сбой чтения коммиченного `.review.yml` не обнуляет домашние слои.** `resolve_policy_data`
  ...
  подполям), списки и скаляры всегда заменяются целиком, а не сливаются поэлементно. `sources` и
  ...
  сравнение проводится только когда `ref` резолвится в тот же коммит, что HEAD клона, иначе статус
  `ref_not_head` явно называет причину пропуска.
