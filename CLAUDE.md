...
- **Недоступность хранилища — классифицированный сигнал, и решает её ТИП исключения (PRI-268).**
  ...
  default и запись о пропуске, не трогая источник. Во-вторых, **`gap()` теперь всегда 4-ключевой**
  (`section`, `reason`, `cause`, `remedy`): потребитель обязан ветвиться по `cause`
