...
- **Недоступность хранилища — классифицированный сигнал, и решает её ТИП исключения (PRI-268).**
  ...
  **Замыкание ловит только то, что до него доходит: fail-soft выше по стеку крал у него
  ...
  а не голый `OperationalError`: это машинный контракт с PRI-274, которая собирается вывести
  PoolTimeout из `is_storage_unavailable`, — тогда тест покраснеет и заставит согласовать
  предикат, вместо того чтобы молча вернуть два таймаута. Ожидать после фикса ровно 30 с
