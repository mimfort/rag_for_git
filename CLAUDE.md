...
- **Секция `code` контекста задачи идёт мультизапросом, и её финальный ранкер — RRF, а не реранкер
  ...
  **реранка и cliff-отсечки в этом пути нет намеренно**: cliff считался по скорам реранкера против
  ...
  сохранить сам дефект. Во-вторых, **текст каждого блока обрезается по границе строк до
  `MAX_BLOCK_CHARS = 2000`** (четверть `max_tool_result_chars`), потому что `as_context` режет рендер
  тупым `text[:8000]` и один чанк-класс на 400 строк выжигал весь символьный бюджет; `as_context`
  общий с ревью PR и потому не меняется — обрезка живёт в `multiquery.py`. В-третьих,
...
- **Бюджет секции `code` файловый, и символьный потолок у него производный (PRI-256).**
  ...
  (`max_files × chars_per_file × 3 // 2`) — это **не** операционный бюджет, а страховочный
