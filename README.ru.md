...
- **Лежащее хранилище — сигнал, а не молчание:** когда Postgres или Neo4j недоступен, первая же
  упавшая секция замыкает остальные — каждая оставшаяся получает свой default и запись в `gaps`
  вместо собственного 30-секундного таймаута пула. *Класс* недоступности решается по типу
  исключения (`is_storage_unavailable`), а причина *внутри* этого класса — неверный пароль или
  несуществующая база, в отличие от остановленных контейнеров — решается по тексту
  (`classify_storage_failure`): при сбое на этапе установления соединения libpq не возвращает
  результат, поэтому SQLSTATE пуст и ветвиться по коду ошибки нельзя. Сырой текст доезжает до
  вызывающего только там, где класс назвать не удалось, и то лишь вымаранным
  (`reviewer/storage_health.py`). Каждая запись `gaps` несёт `cause` (`storage_unavailable` |
  `unknown`), `cause_detail` (`auth_failed` | `missing_database` | `null`) и `remedy` — пустой не
  только для удалённых хранилищ, но и когда причина уже названа: контейнеры и так подняты, и
  лекарство неприменимо, — поэтому скилл ветвится по машиночитаемым полям, а не по прозе. Сервер
  контейнеров не поднимает — он называет лекарство, а `solve-task` спрашивает пользователя,
  запускать ли его. Тот же флаг не даёт `index_batch` обходить мёртвый пул по задаче за раз и
  тратить квоту Voyage, которую некуда записать.
