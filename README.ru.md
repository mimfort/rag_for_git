...
git-фолбэк по ключу задачи). Это РЕЗЕРВ файловых слотов внутри `max_files`, а не потолок на
остаток: гибридный ретрив сначала заполняет весь бюджет `max_files`, и уже по этой итоговой
выдаче — не по сырому пулу ретрива — определяется, известен ли кандидатный путь (по сырому пулу
рычаг терял бы ровно те файлы, ради которых существует). Без кандидатов гибрид получает бюджет
целиком. Второй источник — co-change (git-со-изменяемость файлов) — был реализован, измерен
(4 попадания в ядро на 34 подмешанных пути, просадка bulk-recall) и снят, а не оставлен
выключенным; покрывает только similar-diffs (медиана core-recall 0.5 → 0.75, precision
0.167 → 0.333, 28 попаданий на 35 путей). См. `eval/replay_report.md`, раздел «Приёмка PRI-257».

`metrics.brief_quality.core_paths` (PRI-271) делает линейку ядра продакшн-путей онлайн-метрики
качества брифа свойством репозитория, а не хардкодом: список glob-паттернов (`**` пересекает
`/`, одиночные `*`/`?` — нет), который любой репозиторий настраивает под собственную раскладку
продакшн-кода. Без ключа метрика откатывается на дефолтную линейку этого репозитория
(`DEFAULT_CORE_PATHS` в `reviewer/metrics/brief_quality/config.py`) и отчитывается
`configured: false` — метрика отличает это от явного, но пустого списка, см. раздел о странице
«Quality» ниже.
...
Если `XDG_CONFIG_HOME` не задан, home-root — `~/.config/rag-reviewer`. Слияние **рекурсивно по
mapping-значениям**: слой, не высказавшийся о подсекции, её не стирает — домашний
`context_limits: {graph: {hops: 2}}` сохраняет коммиченный пин `context_limits.code_section`.
Всё остальное заменяется целиком: списки, скаляры, явный пустой mapping, `null` и любая смена
типа. `task_board` — единственный атомарный mapping-ключ: это связный контракт (`type` +
`project` + `create_target`/`done_target` + `options`), поэтому более поздний слой заменяет его
целиком, а не по подполям. Замена значения, заданного и более ранним слоем, называется
**shadowing**. Посмотреть effective policy, источник каждого ключа и перекрытые источники можно
так:
...
`sources` и `shadowed` ключуются **путём до листа через точку**
(`context_limits.code_section.max_files`), а не верхним ключом политики — иначе «подсекция
затенена» неотличимо от «подсекции и не было». У `task_board` и у любого ключа, заданного слоем
целиком (скаляром или `null`), запись одна — на самом ключе. В текстовом выводе ключ, все листья
которого пришли из одного слоя, остаётся одной строкой `source:`; при расхождении слоёв строка
читается `source: mixed`, и под ней печатается по строке на лист.
...
# committed: git-blob     ← резолв прошёл без единого сетевого вызова
```

В JSON это ключ `committed_source` (`git-blob` / `vcs`); сам путь к клону не печатается. Метка
называет прочитанный ОБЪЕКТ — коммиченный git-блоб на ref, а не файл в рабочем дереве клона.

Ровно из-за этой разницы `config show` дополнительно сообщает, разошлось ли рабочее дерево с этим
блобом. Ключ `worktree_drift` (в JSON — `{"status": ..., "keys": [...]}`; в тексте — строка
`worktree_drift:` и под ней разошедшиеся ключи) появляется, только если коммиченный слой
действительно прочитан из клона. Статусы: `clean` (значения не разошлись — правка комментария или
переформатирование дрейфом не считается), `drifted` (перечисляются разошедшиеся листовые ключи,
**никогда не значения**), `absent_in_worktree`, `absent_in_blob`, `ref_not_head` (ref не
резолвится в HEAD клона, сравнение не проводилось) и `unknown` (сбой самой диагностики; в
текстовом выводе он молчит). Дрейф — только предупреждение: эффективная политика по-прежнему
целиком из коммиченного блоба, код возврата не меняется.
...
- **Недоступный источник — сигнал, а не молчание:** когда источник недоступен, первая же упавшая
  секция замыкает остальные *секции этого источника* — каждая оставшаяся получает свой default и
  запись в `gaps` вместо собственного 30-секундного таймаута пула. Замыкание ключуется источником —
  `postgres`, `graph`, `embedder`, — поэтому мёртвый эмбеддер не отменяет секции, которым нужен
  только Postgres, и наоборот. *Класс* недоступности решается по типу исключения
  (`is_storage_unavailable`, `is_embedder_unavailable`), а причина *внутри* хранилищного класса —
  неверный пароль или несуществующая база, в отличие от остановленных контейнеров — решается по
  тексту (`classify_storage_failure`): при сбое на этапе установления соединения libpq не
  возвращает результат, поэтому SQLSTATE пуст и ветвиться по коду ошибки нельзя. Единственное
  уточнение, решаемое типом, — исчерпанный пул: `PoolTimeout` остаётся внутри хранилищного класса
  (вывод лишил бы замыкание возможности сработать на первом же сбое) и может нести
  `cause_detail: pool_exhausted`, потому что контейнеры подняты, а настоящее лекарство — больший
  пул или меньший параллелизм. Одного типа там мало: прод ходит в Postgres только через пул, и
  остановленный контейнер приходит тем же `PoolTimeout`, поэтому ветка добавляет наблюдение —
  одноразовое прямое соединение `psycopg.connect(dsn, connect_timeout=2)` мимо пула. Соединилось —
  пул действительно занят; упало — классифицируется исключение самой пробы, и лежачий контейнер
  сохраняет свой `reviewer start`, а протухший пароль по-прежнему читается как `auth_failed`.
  Сырой текст доезжает до вызывающего только там, где класс назвать
  не удалось, и то лишь вымаранным (`reviewer/storage_health.py`). Каждая запись `gaps` несёт
  `cause` (`storage_unavailable` | `embedder_unavailable` | `unknown`), `cause_detail`
  (`auth_failed` | `missing_database` | `pool_exhausted` | `null`) и `remedy` — пустой не только
  для удалённых хранилищ, но и когда причина уже названа: контейнеры и так подняты, и лекарство
  неприменимо, — поэтому скилл ветвится по машиночитаемым полям, а не по прозе. Мёртвый эмбеддер —
  свой класс, а не уточнение хранилищного: Voyage не хранилище, и `reviewer start` его не чинит.
  Троттлинг Voyage (`RateLimitError`) из класса намеренно исключён — на free tier это штатное
  состояние, которое уже отрабатывает `with_voyage_retry`; вместе с ним исключены ошибки самого
  запроса (`InvalidRequestError` 400, `MalformedRequestError` 422, `VideoProcessingError`):
  отвергнутый запрос — не мёртвый сервис, и считать его таковым значило бы молча обнулить
  половину контекста задачи. Сервер контейнеров не поднимает — он
  называет лекарство, а `solve-task` спрашивает пользователя, запускать ли его. Тот же флаг не даёт
  `index_batch` обходить мёртвый пул по задаче за раз и тратить квоту Voyage, которую некуда
  записать.
...
`brief_quality`, которая пополняется в трёх точках (PRI-270): при каждом реальном
`publish_review` (пишет `run_id`), в `finish_task` и командой CLI `reviewer measure-briefs`
(обе последние пишут `run_id = NULL`). Все три идут через одну функцию —
`reviewer.services.brief_quality.measure_and_record`; идентичность строки — `(repo, pr_number,
task_key)`, а не факт прогона ревью, поэтому более ранний съём без ревью и более поздний
`publish_review` обновляют одну и ту же строку, а не плодят две. Если бриф задачи не найден или
в нём вовсе нет секции `## Relevant code` — измерение пропускается: точка не появляется на
графике вместо нуля или ошибки. Присутствующая, но пустая секция — не пропуск, а валидное
измерение с `predicted = 0`.
