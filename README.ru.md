...
- **Мультизапросный ретрив секции `code`:** секции `code` и `test_exemplars` ищутся **набором**
  ...
  до `MAX_BLOCK_CHARS`, чтобы один огромный чанк не выжигал весь бюджет рендера. Публичный тул
  `search_codebase` остаётся однозапросным и неизменным, как и `Retriever.search_base`; секция
  `subsystems` по-прежнему получает один запрос.
