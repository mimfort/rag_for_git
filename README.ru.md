...
медиану core-recall и precision, bulk-подвыборку (задачи с `expected_core >= 10`) с горизонталью
...
`MCPReviewService`, не отдельный процесс). Если бриф задачи недоступен или его секция
`## Relevant code` пуста — измерение просто пропускается: точка не появляется на графике вместо
нуля или ошибки.
