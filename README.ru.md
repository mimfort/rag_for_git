...
- **Лежащее хранилище — сигнал, а не молчание:** когда Postgres или Neo4j недоступен, первая же
  упавшая секция классифицирует исключение **по типу** (`reviewer/storage_health.py` — никогда по
  его тексту: там может быть DSN с паролем) и замыкает остальные — каждая оставшаяся получает свой
  default и запись в `gaps` вместо собственного 30-секундного таймаута пула. Каждая запись `gaps`
  несёт `cause` (`storage_unavailable` | `unknown`) и `remedy` (команду-лекарство или `null`, если
  хранилища удалённые), поэтому скилл ветвится по машиночитаемому классу, а не по прозе. Сервер
