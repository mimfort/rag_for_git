...
# Тело SKILL.md подставляет $ARGUMENTS в бэктиках прямо перед "is either:"
# (см. plugin/skills/solve-task/SKILL.md, раздел "## Inputs"):
#   `$ARGUMENTS` is either:
#   - a task key (e.g. `PRI-4`, matching the board's `key_pattern`), or
# Ниже по тексту того же тела лежит статический пример `PRI-4` — поиск ключа
# по всему сообщению ловил бы его и для свободнотекстовых вызовов без ключа.
# Поэтому ключ ищем ТОЛЬКО внутри подставленного аргумента.
_ARG_RE = re.compile(r"`([^`]*)`\s*is either:", re.DOTALL)
...
def _extract_key(text: str) -> str | None:
    """Ключ задачи из подставленного $ARGUMENTS вызова скилла, не из всего тела."""
    arg_match = _ARG_RE.search(text)
    if not arg_match:
        return None
    key_match = _KEY_RE.search(arg_match.group(1))
    return key_match.group(1).upper() if key_match else None
...
def window_start(lines: list) -> tuple:
    ...
    for index, line in enumerate(lines):
        ...
        if BASE_DIR_MARKER not in text or SKILL_MARKER not in text:
            continue
        return (_extract_key(text), index)
    return (None, -1)


def find_windows(lines: list) -> list:
    """Все вызовы скилла solve-task транскрипта, по порядку появления.

    Одна сессия может содержать несколько вызовов подряд (разные задачи);
    каждому вызову соответствует своё окно, а не только первому.

    Returns:
        список (ключ задачи или None, индекс начала, индекс конца-исключительно);
        конец окна — начало следующего вызова либо длина транскрипта для последнего.
    """
    starts: list = []
    for index, line in enumerate(lines):
        if line.get("type") != "user":
        ...
        if BASE_DIR_MARKER not in text or SKILL_MARKER not in text:
            continue
        starts.append((_extract_key(text), index))
    windows: list = []
...
def scan_transcripts(root: pathlib.Path) -> dict:
    ...
    Окно без распознанного ключа задачи в измерение не входит. Несколько
    окон одной задачи (разные транскрипты или несколько вызовов в одном)
    суммируются, их число сохраняется как "sessions".
    """
    result: dict = {}
    if not root.exists():
        return result
    for path in sorted(root.rglob("*.jsonl")):
        ...
        if not lines:
            continue
        for key, start, end in find_windows(lines):
            if not key:
                continue
            buckets = aggregate_after(lines[:end], start)
