...
def run_replay(
    *,
    ...
    `config` обязателен: до Task 9 корневой вызывающий (`__main__.py`) передаёт
    сюда `BriefQualityConfig()`, а не полагается на дефолт — молчаливый дефолт
    именно тот тихий провал, ради починки которого затевалась вся задача.
    """
