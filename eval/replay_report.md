...
Прогон от 2026-08-22T14:03:26.302787+00:00, репозиторий `mimfort/rag_for_git`, ветка `dev`.

## Идентичность прогона

- **до**: вариант `baseline`, коммит `9d094a96e497d546a9db0dd3ca1f89e5e5a16bf7`, indexed_sha `a953bed55e8dee8ac5679da7f7f2a86718db4761`, корпус 65
- **после**: вариант `baseline`, коммит `9d094a96e497d546a9db0dd3ca1f89e5e5a16bf7`, indexed_sha `a953bed55e8dee8ac5679da7f7f2a86718db4761`, корпус 65

## Агрегат

| Метрика | до | после | Δ |
|---|---|---|---|
| core-recall (медиана) | 0.25 | 0.25 | +0 |
| core-recall (среднее) | 0.2765 | 0.2765 | +0 |
| core-recall bulk (ядро ≥ 10) | 0.1429 | 0.1429 | +0 |
| bulk N | 5 | 5 | +0 |
| context-recall (медиана) | 0 | 0 | +0 |
| context измерено | 31 | 33 | +2 |
| context без точки измерения | 18 | 16 | -2 |
| union-precision (медиана) | 1 | 1 | +0 |
| precision (медиана) | 1 | 1 | +0 |
| предсказано файлов (медиана) | 2 | 2 | +0 |
| задач измерено | 49 | 49 | +0 |
...
|---|---|
| measured | 33 |
| empty_context_denominator | 12 |
| undefined_context_denominator | 14 |
| context_retrieval_failed | 0 |

## Дельта по задачам

| Ключ | Статус | до | после | Δ | приобретено | потеряно |
|---|---|---|---|---|---|---|
| _и ещё 65_ | без изменений | — | — | — | — | — |
...
**Устарело с PRI-265: ручные разделы больше не сохраняются вручную.** Прогон переписывает
только автоген (шапку, идентичность прогона, агрегат, дельту по задачам) — всё, что стоит ниже
строки-маркера `<!-- generated:end … -->`, включая этот раздел, переносится дословно. Прежняя
процедура (сохранить файл до прогона, вернуть разделы приёмки после) не нужна; если маркер из
файла пропал, прогон откажется стартовать, а не молча сотрёт разбор.
...
Устарело с PRI-265: разделы приёмки прошлых задач стоят ниже строки-маркера
`<!-- generated:end … -->` и переносятся прогоном дословно — вручную восстанавливать нечего.
...
Устарело с PRI-265: разделы приёмки прошлых задач (включая этот) стоят ниже строки-маркера
`<!-- generated:end … -->` и переносятся прогоном дословно. Прежняя ручная процедура
восстановления (Task 6 брифа PRI-259) больше не применяется.
...
## Приёмка PRI-262 — сидирование по изменённым строкам

**Гейт взят: 64.0 % осмысленных путей контекстного ядра на ручной сверке против
предзарегистрированного порога ≥ 50 % и против 41.8 % у PRI-261.** Числа этого раздела
сравнимы только внутри себя (конвенция всех разделов).

### 1. Условия замера

Обе стороны — один `indexed_sha=a07405c7f11b029f30ae0be969fa36a009074ffb`, ветка `dev`,
7657 чанков, команды с явным `--branch dev`. Сторона «до» — коммит 909e974 (предзарегистрация,
реализации ещё нет), прогнанный из отдельного git worktree; сторона «после» — тот же корпус после
правки сидирования. Между прогонами не различается ничего, кроме кода сидов.

Гейт, выборка, порог и правило остановки зафиксированы в `eval/pri262_preregistration.md`
ДО реализации и ДО единого прогона. Пер-путевой разбор — `eval/pri262_eye_check.md`.

### 2. Что изменено

Гранулярность сида — строка, а не чанк, двумя независимыми механизмами:

1. **Значимость хунка по содержанию.** Левый и правый блоки хунка сравниваются с вымаранными
   строковыми литералами и комментариями (tree-sitter, обе стороны мержа). Равенство означает,
   что исполняемого не изменилось ничего — сида нет вовсе. Позиционного правила здесь
   недостаточно: `help="..."` внутри декоратора ЯВЛЯЕТСЯ кодом.
2. **Фильтр по именам с изменённых строк.** Сосед выживает, только если его простое имя
   прозвучало вызовом или базой класса на изменённой строке. Это чинит god-модули, где
   нетронутое тело задетой функции отдавало в ядро своих callee.

Плюс сид сузился до САМОГО ВНУТРЕННЕГО символа: `chunk_python` вкладывает чанки, и хунк в одном
методе прежде сидировал ещё и весь класс.

### 3. Ручная сверка (6 задач, обе стороны одним проходом)

| Задача | до | после |
|---|---|---|
| PRI-227 | 0/6 | 0/0 (сидов нет) |
| PRI-236 | 0/5 | 0/0 (пустой знаменатель) |
| PRI-215 | 6/35 | 5/11 |
| PRI-221 | 9/32 | 8/10 |
| PRI-251 | 5/8 | 2/3 |
| PRI-249 | 3/13 | 1/1 |
| **Итого** | **23/99 = 23.2 %** | **16/25 = 64.0 %** |

Критерий 2 выполнен: улучшение видно именно на четырёх заявленных задачах. Предзарегистрированное
фальсифицируемое предсказание («PRI-227 обязана дать НОЛЬ сидов») подтвердилось.

**Оговорка, которая режет против чистой истории:** сторона «до» здесь читается как 23.2 %, а не как
опубликованные PRI-261 41.8 %. Разница — не в данных, а в критерии: этот проход не засчитывает
файл, которого на момент задачи не существовало. `reviewer/tasks/boards/asana.py` добавлен
2026-07-25, а PRI-215 смержена 2026-07-24 — восемь «полезных соседних адаптеров» PRI-261 прочитать
было нельзя. Гейт берётся стороной «после» (64.0 %), как и требует критерий 1; но дельта в этой
таблице больше, чем дало бы сравнение с 41.8 % один-в-один.

### 4. Цена: 7 осмысленных путей потеряно

Осмысленные пути: 23 → 16; мусорные: 76 → 9. Потери названы поимённо: PRI-251 лишилась
`index/chunker.py`, `index/models.py`, `gitutil.py` (все три настоящие — `inherit.py` строит рёбра
из вывода `chunk_python`, `build_with_scip` гоняет индексатор через временный worktree);
PRI-215 — `tasks/graph.py`; PRI-221 — `services/branch.py`; PRI-249 — `services/cost_sidecar.py`
и `vcs/base.py`. Механизм не видит зависимости, которую изменённые строки не называют. Это не
дрожание ±1 из критерия 4, а систематическая потеря.

### 5. Корпус целиком (63 задачи, те же два прогона)

| | до | после |
|---|---|---|
| путей контекстного ядра всего | 403 | 85 |
...
**Аддитивность (критерий 5) подтверждена эмпирически, а не только свойством на случайных входах:**
все существовавшие числа побайтово совпали между прогонами.

Число, за которым стоит следить, — покрытие: у 11 задач знаменатель стал неопределённым, медиана
размера ядра — 0. Метрика стала заметно точнее и заметно уже. Приемлем ли этот размен — вопрос о
назначении знаменателя, и ручной сверкой он не решается.

### 6. Воспроизведение

```bash
reviewer index . --ref dev                       # sha a07405c, 7657 чанков
git worktree add <tmp> 909e974                   # сторона «до»
PYTHONPATH=. python -m eval.solve_task_metrics replay --branch dev --repo mimfort/rag_for_git
PYTHONPATH=. python -m eval.solve_task_metrics replay --branch dev --repo mimfort/rag_for_git
```

## Приёмка PRI-266 — покрытие знаменателя context-recall

Два независимых механизма поверх сидирования PRI-262: **имена из шапки символа-сида**
(декораторы, аннотации параметров, дефолты, аннотация возврата, базы классов) как второй
источник `allowed_names` и **отдельный статус `undefined_context_denominator`** для задачи, чьё
изменённое ядро целиком не-Python — у неё сидов не может возникнуть ни при какой настройке
фильтра, и её знаменатель неопределим, а не пуст.

Источник имён — рантайм-режим (`--context-seeds lines|lines+signature`), а не правка кода между
замерами: обе стороны A/B сняты одним исходником `9d094a9` на одном
`indexed_sha=a953bed55e8dee8ac5679da7f7f2a86718db4761`, ветка `dev` (явный `--branch dev`),
7705 чанков, корпус 65.

**Гейт (предзарегистрирован в `eval/pri266_preregistration.md`, коммит `fd82798`) взят по обеим
половинам:**

- покрытие: задач с измеренным контекстным знаменателем **31 → 33** (строго растёт);
- точность ручной сверки: **17/29 = 58.6 %** при пороге ≥ 50 %.

| | до (`lines`) | после (`lines+signature`) |
|---|---|---|
| всего путей контекстного ядра | 85 | 99 |
| медиана размера измеренного ядра | 2 | 2 |
| `measured` / `empty` / `undefined` / `failed` | 31 / 14 / 14 / 0 | 33 / 12 / 14 / 0 |
| `core_recall_median` | 0.25 | 0.25 |
| `bulk_core_recall_median` | 0.142857 | 0.142857 |
| `union_precision_median` | 1.0 | 1.0 |
| `n_measured` (core) | 49 | 49 |

Аддитивность подтверждена эмпирически: вся core-линия побайтово совпадает на обеих сторонах.
Иначе и быть не могло — правка целиком живёт в `eval/`, `reviewer/metrics/brief_quality/**` не
тронут, `brief_quality` I/O не получил.

**Вердикт не гладкий, и три вещи стоит знать.** Во-первых, предзарегистрированное предсказание
о PRI-251 **не подтвердилось**: ни один из трёх поимённо потерянных путей (`index/chunker.py`,
`index/models.py`, `gitutil.py`) не вернулся — эти зависимости назывались в телах нетронутых
строк, куда шапка символа не дотягивается. Сигнатурная гипотеза в предзарегистрированной форме
не подтверждена; взято покрытие, а не она. Во-вторых, **сами подмешанные рычагом пути осмысленны
на четверть** (1 из 4 на выборке), из-за чего доля осмысленных сместилась 64.0 % → 58.6 %:
смещение укладывается в предзарегистрированный пол шума (±1 файл на задачу), гейт держится, но
улучшением точности рычаг не является ни в каком прочтении. Характерный мусор виден на корпусе —
`reviewer/vcs/base.py` подмешался в 4 задачи из 7 изменившихся: это типы из аннотаций половины
кодовой базы, и шапка тянет их независимо от темы задачи. В-третьих, **покрытие выросло за счёт
двух задач**, у которых знаменатель был пуст, а стал измерен: PRI-202 (0 → 2) и PRI-247 (0 → 3).

Дефолт режима сидов переведён на `lines+signature` — ровно потому, что гейт взят; при невзятом
гейте стоп-правило требовало оставить `lines` и зафиксировать отрицательный результат.

Полный разбор с пер-путевыми решениями, тремя фальсифицируемыми предсказаниями и ценой размена —
`eval/pri266_eye_check.md`.
