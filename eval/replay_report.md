...
быть осмысленными**. Полный разбор — `.superpowers/sdd/2026-08-19-pri-261-context-core-denominator/eye-check.md`.
...
`--baseline last`) дали ТОЖДЕСТВЕННЫЕ агрегаты и побайтово идентичные `context_core_paths` во всех
трёх. Вывод: ни одна метрика, считающая пути, не может отличить глубину фрагмента; пол
...
`predicted_paths` (не контекстное ядро) отличались на ±1 файл у 4 из 62 задач между прогонами на
ОДИНАКОВЫХ параметрах — причём прогоны 780 и 1300 разошлись друг с другом ровно настолько же,
насколько каждый разошёлся с прогоном 975, немонотонно по `chars_per_file`. Такой рисунок
