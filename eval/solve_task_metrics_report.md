...
Срез от 2026-08-14T17:33:18.230708+00:00, коммит `d474e02656408dc9261430b0e69f048f24f45780`, режим окна замера цены: `sealed`.
...
| С блоком токенов | 29 |
| С ключом задачи | 50 |
| С ground truth (PR-мерж найден) | 44 |
| Отброшено sync-мержей | 11 |

## Цена этапа

- Взвешенный input-эквивалент (основная метрика): **693.3K** (медиана)
- Сырая сумма токенов — **не пропорциональна стоимости**, справочно: 3.05M (медиана); завышает в 4.4×

## Качество ретрива

- core-recall: медиана 56%, среднее 60%, N=35
- Задач без точки измерения (пустой знаменатель ядра): 9 — не считаются нулевым recall
- Медианный размер знаменателя ядра: 4
- core-recall на bulk-подвыборке (ядро ≥ 10 файлов): медиана 37%, N=4
- Сырой recall (справочно, измеряет выбор знаменателя, не качество ретрива): медиана 18%

## Полная цена задачи «под ключ»

- Измерено задач: 21 (остальные — транскрипт недоступен локально, это не ноль, а отсутствие замера)
- Взвешенный input-эквивалент: медиана 6.23M

## Промахи по категориям

| Категория | Промахов |
|---|---|
| новый файл (не существовал до PR) | 385 |
| tests/ | 237 |
| прочее | 114 |
| plugin/skills/*.md | 37 |
| reviewer/tasks | 30 |
