...
class MCPReviewService:
    ...
    def _compute_family(self, repo: str, node_id: str, branch: str):
        ...
        Цена: ``class_members`` и ``bases_of`` тянут ВСЕ классы репо/ветки —
        структурное сопоставление по определению глобально, кандидатом может
        быть любой класс. Отсюда же двойной вызов ``implementations_detailed``
        на пустом ответе ``implementations``: тот считает прямых наследников
        сам, а сюда приходит за семейством. Оба запроса дешевле любого
        обращения к Voyage и на порядок дешевле полноты ответа, ради которой
        задача и делалась; кэш не вводится, пока не появится замер, его
        оправдывающий.
        """
