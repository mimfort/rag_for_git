...
def extract_inheritance_edges(files, chunks_by_path, name_to_nodes,
    ...
    fallback по простому имени. Последний шаг — сознательное приближение: база
    без импорта и без локального определения сопоставляется с ЛЮБЫМ символом
    того же простого имени в индексе, поэтому редкая коллизия имён даёт лишнее
