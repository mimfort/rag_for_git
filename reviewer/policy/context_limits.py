...
class CodeSectionLimits:
    ...
    def max_chars(self) -> int:
        ...
        Здесь же полуторный запас на накладные рендера (префиксы номеров
        строк ~10 %, заголовки блоков ~4 %) и на страховочный случай: cap_block
        удерживает первую строку целиком, даже если она длиннее лимита.
        Без запаса срез в as_context вернул бы ровно тот дефект, который
        файловый бюджет убирает.
        """
        return self.max_files * self.chars_per_file * 3 // 2
