...
|---|---|---|---|---|---|
| 2026-06-12 | minimax/minimax-m3 | mimfort/rag_for_git#2 (5 файлов) | 81→51 (−37%) | $0.4341→$0.2559 (−41%) | 0→0 |

> Условия: один и тот же PR, `--dry-run`. «До» — ветка `main` (cap tool-loop 40), «после» — ветка плана (cap 15 + рычаги A–D). Input-токены 4.70M→1.73M (−63%) — основной вклад граф-смежного bundle (C) и прицельного loop (D). Находок 0→0: PR — собственные хорошо отревьюенные изменения плана, реальных проблем нет, поэтому этот прогон валидирует стоимость, но не качество локаций (ложных `file:line` не на чем проверить — находок нет). Качественную цель (грунтовка строки) закрывают unit-тесты Part A. Замер на крупном «шумном» PR (как исходный 39-файловый на $2.50) при необходимости — отдельным прогоном.
