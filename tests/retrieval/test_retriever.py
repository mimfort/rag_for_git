...
class StoreOnlyHits:
    ...
    def hybrid_search(self, **kw): return [R("a.py#f", "alpha"), R("b.py#g", "beta")]
    def fetch_nodes(self, node_ids, overlay_ref, changed_paths): return []


class GraphEmpty:
    """Граф не расширяет — expand возвращает пустое множество."""
    def expand(self, ids, hops=2): return set()
...
class GraphNewOne:
    ...
    def expand(self, ids, hops=2): return {"d.py#i"}
...
def test_retrieve_calls_rerank_when_candidates_gt_top_k():
    ...
    pack = r.retrieve(query="find f", changed_node_ids=[],
