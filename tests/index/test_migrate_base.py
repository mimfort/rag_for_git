...
def _schema_dsn(dsn: str, schema_name: str) -> str:
    ...
    params["options"] = " ".join(
        option
        for option in (current_options, f"-csearch_path={schema_name}")
        if option
    )
    return make_conninfo(**params)


def _create_private_schema(dsn: str, schema_name: str) -> None:
    with psycopg.connect(dsn) as conn:
        ...
        conn.execute(
            sql.SQL(
                ...
                """
            ).format(sql.Identifier(schema_name, "chunks"))
        )
        conn.execute(
            sql.SQL(
                """
                ...
                """
            ).format(sql.Identifier(schema_name, "index_meta"))
        )
        conn.commit()
...
def _public_bm25_index(store: ChunkStore) -> str | None:
...
def _assert_public_sentinel(store: ChunkStore, repo: str) -> None:
    ...
    assert _public_bm25_index(store) is not None


@pytest.fixture
def migration_context(monkeypatch):
    settings = Settings()
    ...
    try:
        ...
        assert _public_bm25_index(public_store) is not None

        _create_private_schema(settings.pg_dsn, schema_name)
        isolated_schema_created = True

        monkeypatch.setattr("reviewer.index.store.register_vector", lambda _conn: None)
        isolated_store = ChunkStore(_schema_dsn(settings.pg_dsn, schema_name))
        with psycopg.connect(isolated_store.dsn) as conn:
            assert conn.execute("SHOW search_path").fetchone() == (schema_name,)
