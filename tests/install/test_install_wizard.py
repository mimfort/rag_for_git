...
def test_env_template_mirrors_env_example():
    ...
    assert example_keys - template_keys == test_only_keys, (
    ...
    assert not template_keys - example_keys, (
        f"В .env.example нет ключей ENV_TEMPLATE: {sorted(template_keys - example_keys)}"
    )
